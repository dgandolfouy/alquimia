...
// We keep a minimal category-like structure for the 'Element' dropdown if needed, 
// but mostly we use the Enum now.
...
export const FEELING_OPTIONS: { [key in TransactionFeeling]: { label: string, color: string, icon: string } } = {
    necessary: { label: 'Necesario', color: 'bg-gray-400 dark:bg-gray-500', icon: 'Meh' }, // Changed to Meh
...
export const DEFAULT_TRANSMUTATION_LISTS: TransmutationList[] = [
    ...
    },
    {
        ...
        name: 'Supermercado',
        items: [
            { id: 'item-1-1', name: 'Surtido Mensual', amount: 5000, isCompleted: false },
        ]
    },
    {
        id: 'list-2',
        name: 'Gastos Fijos',
        items: [
            { id: 'item-2-1', name: 'Alquiler', amount: 15000, isCompleted: false },
            { id: 'item-2-2', name: 'Internet', amount: 1200, isCompleted: false },
        ]
    }
];

export const ELEMENT_DEFINITIONS: { [key in AlchemicalElement]: { keyword: string, description: string, color: string } } = {
    Tierra: { keyword: 'Base', description: 'Gastos estructurales y necesarios', color: 'text-emerald-500' },
    Agua: { keyword: 'Vida', description: 'Gastos variables para tu estilo de vida', color: 'text-blue-500' },
    Aire: { keyword: 'Volátil', description: 'Antojos, suscripciones, gastos pequeños', color: 'text-yellow-500' },
    Fuego: { keyword: 'Pasión', description: 'Ocio, hobbies, experiencias', color: 'text-rose-500' },
};