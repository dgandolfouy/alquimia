    ...
    <header className="fixed top-0 left-0 right-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-lg border-b border-gray-200/50 dark:border-gray-800/50 z-30 h-24 flex items-center justify-between px-6 transition-all duration-300">
      ...
      <div className="flex items-center gap-3">
        <button 
          ...
          aria-label={isPrivacyMode ? "Mostrar saldo" : "Ocultar saldo"}
        >
        ...
        >
          <Settings 
            size={20} 
            className={`transition-transform duration-500 ease-in-out ${isRotating ? 'rotate-180' : 'rotate-0'}`} 
          />
        </button>
        
        <button 
          onClick={onLogoutClick} 
          className="p-2 text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors rounded-full hover:bg-gray-100 dark:hover:bg-gray-800"
