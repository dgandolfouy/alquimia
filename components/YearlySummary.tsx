...
const YearlySummary: React.FC<YearlySummaryProps> = ({ transactions }) => {
  ...
  if (chartData.length === 0) {
    return null; // Don't render the card if there's no data for the year
  }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-violet-500">{`$${value.toFixed(2)} (${percent}%)`}</p>
        </div>
      );
    }
    return null;
            ...
            <ul className="space-y-1">
                {chartData.map((entry, index) => (
                     <li key={`legend-${index}`} className="flex items-center justify-between">
                        ...
                        <span className="font-mono text-gray-700 dark:text-gray-200">${entry.value.toFixed(0)}</span>
                    </li>
                ))}
            </ul>
        </div>
